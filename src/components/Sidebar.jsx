...
import { clearStoredToken, getStoredRole, getStoredUsername } from '../utils/jwtHelper';
...
const IconFolder = () => (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" /></svg>);
...
const IconList = () => (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>);
...
export default function Sidebar() {
  ...
  const role = getStoredRole() || 'EMPLOYEE';
  const username = getStoredUsername();

  const handleLogout = () => { clearStoredToken(); navigate('/login'); };
  ...
  return (
    <aside className="w-64 min-h-screen bg-primary-600 flex flex-col flex-shrink-0">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {role === 'ADMIN' ? (
          <>
            <NavLink to="/admin" end className={linkClass}><IconDashboard /><span>Dashboard</span></NavLink>
            <NavLink to="/admin/ipr-returns" className={linkClass}><IconList /><span>All IPR Returns</span></NavLink>
            <NavLink to="/admin/employees" className={linkClass}><IconUsers /><span>Employee List</span></NavLink>
            {/* ✅ NEW: Filing Window Management link */}
            <NavLink to="/admin/notifications" className={linkClass}><IconBell /><span>Filing Window</span></NavLink>
          </>
        ) : (
          <>
            <NavLink to="/dashboard" end className={linkClass}><IconDashboard /><span>Dashboard</span></NavLink>
            <NavLink to="/profile" className={linkClass}><IconUser /><span>My Profile</span></NavLink>
            <div>
              <button onClick={() => setIprOpen(o => !o)}
                className="sidebar-link w-full justify-between text-blue-100 hover:bg-primary-700 hover:text-white">
                <span className="flex items-center gap-3"><IconFolder /><span>IPR / Property</span></span>
                {iprOpen ? <ChevronDown /> : <ChevronRight />}
              </button>
              {iprOpen && (
                <div className="ml-4 mt-1 space-y-1 border-l border-primary-500 pl-3">
                  <NavLink to="/ipr/new" className={linkClass}><IconDoc /><span>IPR Filing</span></NavLink>
                  <NavLink to="/submissions" className={linkClass}><IconList /><span>My Submissions</span></NavLink>
                  <NavLink to="/previous-returns" className={linkClass}><IconList /><span>Previous Returns</span></NavLink>
                </div>
              )}
            </div>
            <NavLink to="/help" className={linkClass}><IconHelp /><span>Help &amp; Support</span></NavLink>
          </>
        )}
      </nav>
      ...
      <div className="px-4 py-4 border-t border-primary-700">
        ...
        <button onClick={handleLogout} className="w-full text-left text-blue-200 hover:text-white text-sm flex items-center gap-2 py-1">
