...
// Matches exact Employee entity fields
const EMPTY_FORM = {
  name: '',
  email: '',
  service: '',
...
function EmployeeModal({ initial, onSave, onClose }) {
  const [form, setForm] = useState(initial ? {
    ...
    presentPostHeld: initial.presentPostHeld || '',
    placeOfPosting: initial.placeOfPosting || '',
    lengthOfService: initial.lengthOfService || '',
  } : EMPTY_FORM);
  ...
  const handleSubmit = async (e) => {
    ...
    catch (ex) { setErr(ex?.response?.data?.message || 'Failed to save employee.'); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h3 className="font-semibold text-gray-800">{initial?.id ? 'Edit Employee' : 'Add New Employee'}</h3>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
          ...
          <div>
            ...
            <input type="email" name="email" value={form.email} onChange={handle} className="form-input" placeholder="Official email address" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" name="presentPostHeld" value={form.presentPostHeld} onChange={handle} className="form-input" placeholder="e.g. Assistant Director" />
            </div>
            <div>
...
export default function AdminEmployees() {
  ...
  const handleSave = async (formData) => {
    if (editing?.id) {
      const res = await updateEmployee(editing.id, formData);
      setEmployees((prev) => prev.map((e) => e.id === editing.id ? res.data : e));
    } else {
    ...
    <Layout title="Employee Management">
      ...
      <div className="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Add and manage all employee records</p>
        </div>
      ...
      <div className="card">
        <div className="px-5 py-3 border-b border-gray-200">
          <input type="text" placeholder="Search by name, department, service..."
            value={search} onChange={(e) => setSearch(e.target.value)} className="form-input w-72" />
        </div>
        {loading ? (
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <th className="table-header">Present Post Held</th>
              ...
              <tbody>
                {filtered.map((emp, idx) => (
                  <tr key={emp.id || idx} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell">{emp.presentPostHeld || '—'}</td>
