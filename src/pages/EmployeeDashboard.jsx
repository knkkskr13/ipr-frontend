...
import { getAllIprReturns, deleteIprReturn } from '../api/iprApi';
...
export default function EmployeeDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [profileRes, iprRes] = await Promise.all([
          getMyProfile(),
          getAllIprReturns(),
        ]);
    ...
    <Layout title="Employee Dashboard">
      ...
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-800">
          Welcome, {profile?.name || profile?.employeeName || 'Employee'}
        </h2>
        ...
        <div className="card p-5 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              ['Present Post Held', profile.presentPost || profile.presentPostHeld],
      ...
      <div className="card">
        ...
        {iprReturns.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-sm">No IPR returns found. Click "New IPR Return" to get started.</p>
          </div>
        ) : (
                      ...
                      <div className="flex items-center gap-2">
                        {ipr.status === 'DRAFT' && (
                          <>
                            ...
                            <button
                              onClick={() => handleDelete(ipr.iprId)}
                              className="text-xs bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-2.5 py-1 rounded font-medium"
                            >
                              Delete
                            </button>
                          </>
                        )}
                        {(ipr.status === 'SUBMITTED' || ipr.status === 'APPROVED' || ipr.status === 'RETURNED') && (
                          <button
                            onClick={() => navigate(`/ipr/${ipr.iprId}`)}
